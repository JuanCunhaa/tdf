...
router.get('/', async (_req, res) => {
  ...
  const stats = await prisma.userStats.groupBy({ by: ['user_id'], _sum: { rank_points: true, goals_completed: true } });
  ...
  const topByGoals = [...stats]
    ...
    .slice(0, 20);
  res.json({ topByRankPoints: topByRank, topByGoalsCompleted: topByGoals });
});
